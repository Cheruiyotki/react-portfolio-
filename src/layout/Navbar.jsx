...
export const Navbar = () => {
    ...
    return (
  <header className={`fixed top-0 right-0 left-0 transition-all duration-300 ${
    isScrolled ? "glass-strong py-3" : "bg-transparent py-5"} 
     z-50`}>
        <nav className="container mx-auto px-6 flex items-center justify-between">
            
            <a href="#" className="text-xl font-bold tracking-tight hover:text-primary" >WK
        ...
        { isMobileMenuOpen && (
          <div className="md:hidden glass-strong animate-fade-in">
            <div className="container mx-auto px-6 py-6 flex flex-col gap-4 ">
              {navLinks.map((link, index  ) => (
                      <a key={index} href={link.href} className="text-lg text-muted-foregroung hover:text-foreground py-2"> {link.label}</a>
                    ))}
                     <Button size="sm">
                Contact Me
               </Button>
